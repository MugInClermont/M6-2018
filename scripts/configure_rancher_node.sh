...
while true; do
  ENV_STATE=$(docker run \
    ...
    appropriate/curl \
      ...
      "https://$rancher_server_ip/v3/clusterregistrationtoken?name=$cluster_name" | jq -r '.data[].nodeCommand')
...
CLUSTERRESPONSE=$(docker run --net host \
    --rm \
    appropriate/curl -s "https://$rancher_server_ip/v3/clusters?name=$cluster_name" -H 'content-type: application/json' -H "Authorization: Bearer $LOGINTOKEN" --insecure)
...
AGENTCMD=$(docker run --net host \
    --rm \
    appropriate/curl -s "https://$rancher_server_ip/v3/clusterregistrationtoken?id=$CLUSTERID" -H 'content-type: application/json' -H "Authorization: Bearer $LOGINTOKEN" --insecure | jq -r .data[].nodeCommand  | head -1)
